...
pub fn process_gameplay(
    ...
    if (values.prev_playtime == values.playtime && values.playtime > 0) {
        values.paused = 1;
    }
...
pub fn process_reading_loop(p: &Process, state: &mut State) -> Result<()> {
            ...
            let current_beatmap = match Beatmap::from_path(&values.beatmap.paths.beatmap_full_path)
            {
                Ok(beatmap) => {
                    ...
                    values
                        ...
                        .paths
                        .background_file
                        .clone_from(&beatmap.background.filename);
    ...
    if new_map {
        if let Some(map) = &values.current_beatmap {
            if let Cow::Owned(converted) = map.convert_mode(values.menu_gamemode()) {
